...
#[no_mangle] unsafe extern "C" fn text_text_get(ptr: *mut html_content::TextChunk) -> ffi::mem::buf { return ffi::io::store((*ptr).as_str().as_bytes().to_owned()); }
...
#[no_mangle] unsafe extern "C" fn element_get_attribute(ptr: *mut html_content::Element, bptr: ffi::mem::buf, blen: usize) -> ffi::mem::buf { (*ptr).get_attribute(&ffi::io::string(bptr, blen)).map_or(ffi::ptr::err(0), |x| ffi::io::peek(x.as_bytes())) }
...
#[no_mangle] unsafe extern "C" fn new(id: u64, pump: u8, ptr: ffi::mem::buf, len: usize) -> *mut rewriter {
  ...
  let document_handlers = json.element_handlers.iter().map(|x| {
    let kind = x.kind;
    let index = x.index;
